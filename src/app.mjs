...
import { ASSIGNMENT_STAGES, advanceAssignment, assignmentToCheckin, createAssignment } from "./learning.mjs";
...
function assignmentStatus(item){return item.status||(item.done?"已确认":"待完成");}
function assignmentCard(item){
  const status=assignmentStatus(item); const action={"待完成":["submit","提交作业"],"待搭子确认":["confirm","模拟搭子确认"],"已确认":["checkin","转为成长打卡"]}[status];
  return `<article class="assignment-card"><div class="assignment-copy"><span class="assignment-status">${status}</span><h4>${esc(item.title)}</h4><p>${esc(item.criteria||"完成后由搭子确认学习成果")}</p><small>${esc(item.course||"共学课程")} · ${esc(item.due||"待协商")}</small></div><div class="assignment-steps">${ASSIGNMENT_STAGES.map(stage=>`<span class="${ASSIGNMENT_STAGES.indexOf(stage)<=ASSIGNMENT_STAGES.indexOf(status)?'done':''}">${stage}</span>`).join("")}</div>${action?`<button class="${status==='已确认'?'primary':'ghost'}" data-assignment-action="${action[0]}" data-assignment-id="${item.id}">${action[1]}</button>`:`<a class="checkin-link" href="#/social">✓ 已进入同学圈</a>`}</article>`;
}

function classroomView() {
  const session=state.sessions[0]; const p=person(session?.partnerId)||PEOPLE[0]; const recentNote=state.notes[0];
  frame(`${pageHead("从一起上课，到一起完成", "共学空间")}<div class="classroom-layout"><section class="classroom"><div class="video-stage"><div class="remote-video">${avatarMarkup({...p,avatar:p.avatar},"")}<span class="video-label">${p.name} · 示例远端画面</span></div><div class="local-video"><video id="localMedia" autoplay muted playsinline></video><span class="video-label">我</span></div></div><div class="media-controls"><button class="media-button" data-media="camera">◉ 开启摄像头</button><button class="media-button" data-media="mic">♬ 麦克风</button><button class="media-button" data-media="screen">▣ 共享屏幕</button><button class="media-button stop" data-media="stop">结束媒体</button></div></section><aside class="lesson-side"><section class="panel lesson-card"><div class="section-head"><h3>${esc(session?.title||"技能交换课堂")}</h3><span class="mini-badge">${session?.duration||45} 分钟</span></div><ol class="lesson-outline"><li>确认本节课目标</li><li>搭子演示与跟练</li><li>独立完成一个小任务</li><li>确认课后作业</li></ol><div class="lesson-note-count"><strong>${state.notes.length}</strong><span>条知识笔记已沉淀</span></div></section>${recentNote?`<section class="panel recent-note"><span>最近一条笔记</span><strong>${esc(recentNote.title)}</strong><p>${esc(recentNote.takeaway||recentNote.body)}</p></section>`:""}</aside></div><div class="after-class-grid"><section class="panel learning-workbench"><div class="section-head"><div><p class="eyebrow">课堂沉淀</p><h2>结构化笔记</h2></div><span class="section-note">自动保存到成长档案</span></div><form class="form-grid" data-note-form data-session-id="${session?.id||''}"><label class="field">笔记标题<input name="title" required placeholder="例如：Python 列表处理的三个关键点"></label><label class="field">今天真正学会了什么<textarea name="takeaway" rows="3" required placeholder="用自己的话复述方法和原理…"></textarea></label><div class="form-grid two"><label class="field">还没解决的问题<textarea name="question" rows="2" placeholder="下次课想继续问什么？"></textarea></label><label class="field">下一步练习<textarea name="nextStep" rows="2" required placeholder="课后准备完成的最小行动…"></textarea></label></div><button class="primary">保存这节课的笔记</button></form></section><section class="panel assignment-workbench"><div class="section-head"><div><p class="eyebrow">课后闭环</p><h2>作业与成长打卡</h2></div><span class="section-note">完成后由搭子确认</span></div><form class="assignment-form" data-assignment-form data-session-id="${session?.id||''}"><label class="field">作业任务<input name="title" required placeholder="例如：完成一份自动整理表格的脚本"></label><label class="field">完成标准<input name="criteria" required placeholder="例如：能处理 3 种常见表格格式"></label><label class="field">截止时间<input name="due" required value="本周日前"></label><button class="secondary">＋ 添加课后作业</button></form><div class="assignment-list">${state.assignments.map(assignmentCard).join("")}</div></section></div>`);
}
...
document.addEventListener("click", (event) => {
  ...
  if(target.dataset.assignmentAction){const index=state.assignments.findIndex(item=>item.id===target.dataset.assignmentId);if(index<0)return;const item=state.assignments[index];if(target.dataset.assignmentAction==="submit"||target.dataset.assignmentAction==="confirm"){state.assignments[index]=advanceAssignment({...item,status:assignmentStatus(item)});persist();render();showToast(target.dataset.assignmentAction==="submit"?"作业已提交，等待搭子确认":"搭子已确认这次学习成果");return;}if(target.dataset.assignmentAction==="checkin"){const result=assignmentToCheckin({...item,status:assignmentStatus(item)},state.profile);state.assignments[index]=result.assignment;state.posts.unshift(result.post);if(state.sessions[0])state.sessions[0].status="已复盘";persist();render();showToast("已转为成长打卡，可在同学圈查看");return;}}
...
document.addEventListener("submit", (event) => {
  ...
  if(event.target.matches("[data-note-form]")){event.preventDefault();const data=Object.fromEntries(new FormData(event.target));const body=[data.takeaway,data.question&&`待解决：${data.question}`,data.nextStep&&`下一步：${data.nextStep}`].filter(Boolean).join(" · ");state.notes.unshift({id:`note-${Date.now()}`,sessionId:event.target.dataset.sessionId||null,title:data.title,takeaway:data.takeaway,question:data.question,nextStep:data.nextStep,body,updatedAt:"刚刚"});persist();render();showToast("结构化笔记已保存到成长档案");return;}
  if(event.target.matches("[data-assignment-form]")){event.preventDefault();const data=Object.fromEntries(new FormData(event.target));const session=state.sessions.find(item=>item.id===event.target.dataset.sessionId)||state.sessions[0];state.assignments.unshift(createAssignment({...data,sessionId:session?.id,course:session?.title||"共学课程"}));persist();render();showToast("课后作业已创建，完成后可提交给搭子确认");return;}
