...
const SKILL_ART={code:`<svg viewBox="0 0 80 80" aria-hidden="true"><path class="art-shadow" d="M16 20h48v40H16z"/><rect x="14" y="15" width="52" height="42" rx="10"/><circle cx="23" cy="24" r="3"/><circle cx="32" cy="24" r="3"/><path d="m34 35-8 7 8 7M46 35l8 7-8 7"/></svg>`,video:`<svg viewBox="0 0 80 80" aria-hidden="true"><path class="art-shadow" d="M23 13h34v55H23z"/><rect x="20" y="9" width="40" height="56" rx="12"/><path d="m36 27 15 10-15 10z"/><circle cx="40" cy="57" r="3"/></svg>`,music:`<svg viewBox="0 0 80 80" aria-hidden="true"><path class="art-shadow" d="M28 58c0-9 10-17 20-13l6-29 7 2-7 31c-2 11-17 18-25 11z"/><path d="M25 54c0-9 10-16 20-12l6-29 8 2-8 31c-2 11-17 18-26 8z"/><path d="m48 26 9 3"/></svg>`,photo:`<svg viewBox="0 0 80 80" aria-hidden="true"><path class="art-shadow" d="M13 27h56v38H13z"/><rect x="10" y="23" width="60" height="38" rx="11"/><path d="m24 23 5-9h22l5 9"/><circle cx="40" cy="42" r="13"/><circle cx="40" cy="42" r="6"/><circle cx="60" cy="31" r="3"/></svg>`};
function skillArt(skill){if(/摄影/.test(skill))return SKILL_ART.photo;if(/吉他|尤克里里/.test(skill))return SKILL_ART.music;if(/视频|短视频/.test(skill))return SKILL_ART.video;return SKILL_ART.code;}
function discoveryCard(item,index){return `<button class="skill-discovery tone-${index%4}" data-discover-skill="${esc(item.skill)}"><span class="discovery-art">${skillArt(item.skill)}</span><span class="discovery-copy"><small>探索新技能</small><strong>${esc(item.skill)}</strong><em>${item.mentors} 位搭子能教 · ${item.eveningMentors} 位今晚有空</em></span><span class="discovery-go">→</span></button>`;}
